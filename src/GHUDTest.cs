...
namespace GhostPlayer
{
    ...
    public class GHUDTest : MonoBehaviour
    {
        ...
        void Start()
        {
            try
            {
                DebugHandler.Log("[雨甸中文输入] GHUDTest开始初始化");
                
                // 不在Start中创建GHUD实例，而是在Update中根据游戏状态创建
                initialized = true;
                DebugHandler.Log("[雨甸中文输入] GHUDTest初始化成功");
            }
            catch (Exception ex)
            {
                DebugHandler.LogError("[雨甸中文输入] GHUDTest初始化失败: " + ex.Message, ex);
            }
        }
        ...
        void Update()
        {
            ...
            {
                DebugHandler.LogError($"[雨甸中文输入] GHUDTest更新时遇到KeyNotFoundException: {knfEx.Message}", knfEx);
            }
            catch (Exception ex)
            {
                DebugHandler.LogError("[雨甸中文输入] GHUDTest更新失败: " + ex.Message, ex);
            }
        }
        ...
        private void ManageGHUDInstance()
        {
            try
            {
                ...
                {
                    DebugHandler.Log($"[雨甸中文输入] 游戏主循环类型变化: {lastMainLoopType?.Name ?? "null"} -> {currentType?.Name ?? "null"}");
                ...
                {
                    DebugHandler.Log("[雨甸中文输入] 检测到游戏场景，创建GHUD实例");
                    
                    // 检查是否已存在GHUD对象
                    var existingGHUD = GameObject.Find("GHUD");
                    if (existingGHUD != null)
                    {
                        DebugHandler.Log("[雨甸中文输入] 发现现有GHUD对象，尝试使用");
                        ghud = existingGHUD.GetComponent<GHUD>();
                        
                        // 如果对象存在但组件不存在，则添加组件
                        if (ghud == null)
                        {
                            DebugHandler.Log("[雨甸中文输入] GHUD对象存在但组件不存在，添加组件");
                    ...
                    else
                    {
                        ...
                        DebugHandler.Log("[雨甸中文输入] 已创建新的GHUD实例");
                    }
                }
                // 如果不在游戏内但GHUD实例存在，则不处理（GHUD会自行检测并销毁）
                else if (!isInGame && ghud != null)
                {
                    DebugHandler.Log("[雨甸中文输入] 不在游戏场景中，GHUD将自行检测并销毁");
                    ghud = null; // 清除引用
                }
            }
            catch (Exception ex)
            {
                DebugHandler.LogError("[雨甸中文输入] 管理GHUD实例失败: " + ex.Message, ex);
            }
        }
    }
} 